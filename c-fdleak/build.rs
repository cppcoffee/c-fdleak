...
/// This crate has a runtime dependency on artifacts produced by the `c-fdleak-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "c-fdleak-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `c-fdleak-ebpf`, so cargo will automatically rebuild us
        // if `c-fdleak-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `c-fdleak-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("c-fdleak-ebpf");
